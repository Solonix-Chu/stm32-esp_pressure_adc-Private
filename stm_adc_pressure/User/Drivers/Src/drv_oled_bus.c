#include "drv_oled_bus.h"

#include "main.h"

static inline void drv_oled_sclk_low(void)
{
  OLED_SCLK_GPIO_Port->BSRR = (uint32_t)OLED_SCLK_Pin << 16U;
}

static inline void drv_oled_sclk_high(void)
{
  OLED_SCLK_GPIO_Port->BSRR = OLED_SCLK_Pin;
}

static inline void drv_oled_sdin_low(void)
{
  OLED_SDIN_GPIO_Port->BSRR = (uint32_t)OLED_SDIN_Pin << 16U;
}

static inline void drv_oled_sdin_high(void)
{
  OLED_SDIN_GPIO_Port->BSRR = OLED_SDIN_Pin;
}

static inline void drv_oled_dc_command(void)
{
  OLED_DC_GPIO_Port->BSRR = (uint32_t)OLED_DC_Pin << 16U;
}

static inline void drv_oled_dc_data(void)
{
  OLED_DC_GPIO_Port->BSRR = OLED_DC_Pin;
}

static inline void drv_oled_cs_low(void)
{
  OLED_CS_GPIO_Port->BSRR = (uint32_t)OLED_CS_Pin << 16U;
}

static inline void drv_oled_cs_high(void)
{
  OLED_CS_GPIO_Port->BSRR = OLED_CS_Pin;
}

static inline void drv_oled_rst_low(void)
{
  OLED_RST_GPIO_Port->BSRR = (uint32_t)OLED_RST_Pin << 16U;
}

static inline void drv_oled_rst_high(void)
{
  OLED_RST_GPIO_Port->BSRR = OLED_RST_Pin;
}

static void drv_oled_write_byte(uint8_t value, uint8_t is_command)
{
  if (is_command != 0U)
  {
    drv_oled_dc_command();
  }
  else
  {
    drv_oled_dc_data();
  }

  drv_oled_cs_low();

  for (uint8_t bit = 0U; bit < 8U; ++bit)
  {
    drv_oled_sclk_low();

    if ((value & 0x80U) != 0U)
    {
      drv_oled_sdin_high();
    }
    else
    {
      drv_oled_sdin_low();
    }

    drv_oled_sclk_high();
    value <<= 1U;
  }

  drv_oled_cs_high();
  drv_oled_dc_data();
}

void DrvOledBus_Init(void)
{
  drv_oled_cs_high();
  drv_oled_dc_data();
  drv_oled_sclk_high();
  drv_oled_sdin_high();
  drv_oled_rst_low();
  HAL_Delay(100U);
  drv_oled_rst_high();
  HAL_Delay(10U);
}

void DrvOledBus_WriteCommand(uint8_t command)
{
  drv_oled_write_byte(command, 1U);
}

void DrvOledBus_WriteCommandList(const uint8_t *commands, size_t length)
{
  if ((commands == NULL) || (length == 0U))
  {
    return;
  }

  for (size_t index = 0U; index < length; ++index)
  {
    DrvOledBus_WriteCommand(commands[index]);
  }
}

void DrvOledBus_WriteData(const uint8_t *data, size_t length)
{
  if ((data == NULL) || (length == 0U))
  {
    return;
  }

  for (size_t index = 0U; index < length; ++index)
  {
    drv_oled_write_byte(data[index], 0U);
  }
}
