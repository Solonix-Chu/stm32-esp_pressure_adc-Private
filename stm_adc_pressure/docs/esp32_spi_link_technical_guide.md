...
- 协议头中的 `sample_rate_hz` 字段始终跟随当前运行频率
...
- 总样本数：`1536`
- 每通道样本数：`256`
- 单包覆盖时间：`256 / sample_rate_hz` 秒
- 默认 `100 Hz/ch` 时包产生周期约 `2560 ms`，包频率约 `0.39 包/秒`
- `2000 Hz/ch` 时包产生周期约 `128 ms`，包频率约 `7.81 包/秒`
...
- 从纯吞吐量看，即使升到 `2000 Hz/ch`，链路理论最低速率也只在 `0.20 MHz` 量级。
- 为保守起见，建议先以 `6 ~ 8 MHz` 联调，再视波形质量提升到 `10~12 MHz`。
...
| `16` | `4` | `sample_rate_hz` | 当前运行中的每路采样频率，范围 `100 ~ 2000`，默认 `100` |
...
- 包池深度：`4`
- 每包覆盖时间：`256 / sample_rate_hz` 秒
- 默认 `100 Hz/ch` 时理论连续缓存窗口约 `10.24 s`
- `2000 Hz/ch` 时理论连续缓存窗口约 `512 ms`

这意味着：

- ESP32 端平均处理速度必须高于 `sample_rate_hz / 256` 包/秒
- 默认 `100 Hz/ch` 时约为 `0.39 包/秒`
- `2000 Hz/ch` 时约为 `7.81 包/秒`
- 在高采样率下，如果持续数百毫秒都未能取走数据，STM32 侧就可能开始丢包
...
- 单包：`3112 bytes`
- 平均链路负载：`3112 * sample_rate_hz / 256 bytes/s`
- 默认 `100 Hz/ch` 时约 `1.22 kB/s`
- `2000 Hz/ch` 时约 `24.31 kB/s`

换算成 SPI 线速需求：

- 默认 `100 Hz/ch` 时最低理论速率约 `0.01 MHz`
- `2000 Hz/ch` 时最低理论速率约 `0.20 MHz`
- 为了留出 DMA 调度和时序裕量，仍建议至少从 `6 ~ 8 MHz` 起步联调
